<script lang="ts">
  ...
  {#if results.length > 0}
    <div class="bg-slate-800/70 backdrop-blur-sm rounded-lg border border-slate-700/50 overflow-hidden">
      ...
      <!-- Stats grid -->
      <div class="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-700/50">
        <!-- Severity summary -->
        <div class="p-4">
          ...
          <div class="h-32 flex items-end gap-1">
            {#each ['critical', 'high', 'medium', 'low', 'none'] as severity}
              ...
              <div class="flex-1 flex flex-col items-center">
                ...
                <div class="text-xs text-slate-400 mt-1 capitalize hidden sm:block">{severity.substring(0, 1)}</div>
              </div>
            {/each}
          </div>
        </div>
          ...
          <div class="space-y-2">
            ...
            {:else}
              ...
              <div class="flex flex-wrap gap-2">
                {#each getAffectedPackages().slice(0, 5) as pkg}
                  <span class="px-2 py-1 text-xs bg-slate-700/50 text-slate-200 rounded border border-slate-600/50">
                    {pkg}
                  </span>
                {/each}
                {#if getAffectedPackages().length > 5}
                  <span class="px-2 py-1 text-xs bg-slate-700/50 text-slate-200 rounded border border-slate-600/50">
                    +{getAffectedPackages().length - 5} more
                  </span>
                {/if}
              </div>
            {/if}
