<script lang="ts">
  ...
  <div class="space-y-4 w-full" bind:this={containerRef}>
    <!-- Summary section with counter -->
    <div class="bg-slate-800/70 backdrop-blur-sm sticky top-0 z-10 rounded-lg border border-slate-700/70 p-4">
    ...
    <!-- Pagination -->
    {#if getPageCount() > 1}
      <div class="flex justify-center mt-6 pb-6 sticky bottom-0 bg-gradient-to-t from-slate-900/90 to-slate-900/60 p-4 rounded-lg backdrop-blur-sm">
        <div class="inline-flex rounded-md shadow-sm">
          ...
          <!-- Fast backward for large datasets -->
          {#if currentPage > 3 && getPageCount() > 7}
            <button
              onclick={() => handlePageChange(1)}
              aria-label="Go to first page"
              class="relative inline-flex items-center px-3 py-2 text-sm font-medium border-y border-x border-slate-700 bg-slate-800/70 text-slate-300 hover:bg-slate-700/70"
            >
              1
            </button>
          
            {#if currentPage > 4}
              <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium border-y border-x border-slate-700 bg-slate-800/70 text-slate-400">
                …
              </span>
            {/if}
          {/if}
          ...
          <!-- Page numbers -->
          {#each Array(getPageCount()).fill(0).map((_, i) => i + 1) as page}
            {@const showPage = 
              (getPageCount() <= 7) || 
              (page === 1) || 
              (page === getPageCount()) ||
            ...
            {#if showPage}
              <button
                ...
                class="relative inline-flex items-center px-4 py-2 text-sm font-medium border-y border-x border-slate-700 {currentPage === page ? 'bg-blue-600/30 text-blue-300 z-10' : 'bg-slate-800/70 text-slate-300 hover:bg-slate-700/70'}"
              >
                {page}
              </button>
            {:else if (page === 2 && currentPage > 4) || (page === getPageCount() - 1 && currentPage < getPageCount() - 3)}
              <!-- Skip indicator already shown above -->
            {/if}
          {/each}
          
          <!-- Fast forward for large datasets -->
          {#if currentPage < getPageCount() - 2 && getPageCount() > 7}
            {#if currentPage < getPageCount() - 3}
              <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium border-y border-x border-slate-700 bg-slate-800/70 text-slate-400">
                …
              </span>
            {/if}
